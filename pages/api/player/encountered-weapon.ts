...
export default async function handler(
    ...
    try {
        ...
        for (let itemInfo of items) {
            let simpleWeaponInfo: SimpleEquipableItemInfo = {
                encountered: foundItemInfo.includes(itemInfo.id),
                silhouetteId: itemInfo.silhouetteId,
                battleType: itemInfo.battleType,
            }
            if (simpleWeaponInfo.encountered) {
                simpleWeaponInfo.name = itemInfo.name
                simpleWeaponInfo.description = itemInfo.description
            }
            retItems.push(simpleWeaponInfo)
        }
